...
The {shiny} R package allows users to rapidly set up interactive interfaces to their datasets, allowing them and other to explore the data directly. 
...
This will generate a skeleton for an app. Try runnning it by clicking "Run app". This should start the default shiny application (looking like the image below). Try dragging the "number of bins" slider. The plot should change accordingly.
...
**Note: If you don't want to do this part - jump straight to the next section and load the data preparsed.** It is here as reference if you want to parse your own data.

I have prepared a comma-separated Gapminder dataset on life expectancy from year 1800. If you have your own dataset, exchange the file path for that one instead and adjust the code accordingly. 
...
If you skipped the previous section - load the preparsed data directly. This will load a three-column dataset specifying country, measurement year and estimated life expectancy for that country and year.
...
If you have any trouble with this tutorial, feel free to throw me an email at jakob [dot] willforss [at] hotmail [dot] com. You can also find me on: [GitHub](https://github.com/Jakob37/ShinyFromScratch) and [Linkedin]().
